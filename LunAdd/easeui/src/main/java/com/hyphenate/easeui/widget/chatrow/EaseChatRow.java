...
public abstract class EaseChatRow extends LinearLayout {
    ...
    private void setUpBaseView() {
        ...
        if (message.direct() == Direct.SEND) {
            ...
            if (nick != null && !nick.equals(user.getString(UserDao.OTHERUSERNAME))) {
                user.put(UserDao.OTHERUSERNAME,nick);
                user.saveInBackground();
            }
            usernickView.setText(user.getString(UserDao.OTHERUSERNAME));
        }
        ...
        if (adapter instanceof EaseMessageAdapter) {
            if (((EaseMessageAdapter) adapter).isShowAvatar())
                ...
                userAvatarView.getLayoutParams().width = 0;
        ...
        activity.runOnUiThread(new Runnable() {
            public void run() {
                if (message.status() == EMMessage.Status.FAIL) {
                    Toast.makeText(activity,activity.getString(R.string.send_fail) + activity.getString(R.string.connect_failuer_toast)+activity.getString(R.string.messageblocked), Toast.LENGTH_LONG).show();
                }
